...
export class ListingComponent implements OnInit {
    ...
    parsePopulation(population: string): number {
        if (!population) return 0;
        const populationInMillion = parseFloat(population.replace(/[^\d.-]/g, '')); // Remove non-numeric characters
        return populationInMillion * 1000000; // Convert to absolute number
      }

      parseTerritory(territory: string): number {
        if (!territory) return 0;
        const territoryInKm2 = parseFloat(territory.replace(/[^\d.-]/g, '')); // Remove non-numeric characters
        return territoryInKm2; // Return as number
      }
